...
# OTL 30/3/10
ca1 <- compare.adaptR6$new(func=OTL_Circuit, reps=2, batches=2, D=6, L=3, n0=30, 
                           obj=c("desirability","desirability","desirability","desirability","desirability"), 
                           selection_method=c("nonadapt","nonadapt","nonadapt",'SMED', 'max_des_red'), 
                           des_func=c('des_func_quantile'),#,'des_func_relmax', 'des_func_relmax'), 
                           design=c('random', 'lhd', 'sFFLHD_Lflex', 'sFFLHD_Lflex', 'sFFLHD_Lflex'),
                           alpha_des=1e2, actual_des_func=c(get_actual_des_func_quantile(f=borehole, D=2, threshold=.9, power=1)), 
                           package="laGP_GauPro", seed=76)$run_all()$plot_awe_over_batch()
