...
if (run_bran1) {
  ...
  if (file.exists(bran1_file)) { # Load if saved, and recover
    ...
    bran1 <- compare.adaptR6$new(func='branin', reps=100, batches=10, D=2, L=3,
                                ...
                                design=designs,
                                des_func=,
                                actual_des_func='actual_des_func_grad_norm2_mean_branin',
...
if (run_franke1) {
  ...
  if (file.exists(franke1_file)) { # Load if saved, and recover
    ...
    franke1 <- compare.adaptR6$new(func='franke', reps=100, batches=10, D=2, L=3,
                                  ...
                                  design=designs,
                                  des_func=des_funcs,
                                  actual_des_func='actual_des_func_grad_norm2_mean_franke',
                                  alpha_des=1, weight_const=0,
                                  package="laGP_GauPro_kernel",
                                  error_power=2,
                                  seed_start=10, design_seed_start=100,
...
if (run_lim1) {
  ...
  if (file.exists(lim1_file)) { # Load if saved, and recover
    ...
    lim1 <- compare.adaptR6$new(func='limnonpoly', reps=100, batches=10, D=2, L=3,
                                ...
                                design=designs,
                                des_func=des_funcs,
                                actual_des_func='actual_des_func_grad_norm2_mean_limnonpoly',
                                alpha_des=1, weight_const=0,
                                package="laGP_GauPro_kernel",
                                error_power=2,
                                seed_start=10, design_seed_start=100,
...
bran1 <- run_test(funcstring='branin', D=2, L=3, batches=5, reps=5, stage1batches=2, seed_start=123, design_seed_start=456, startover = T)
