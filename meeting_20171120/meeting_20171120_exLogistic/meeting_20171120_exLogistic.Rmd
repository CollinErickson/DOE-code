---
title: "Meeting 11/20 Example Logistic"
...
* Added 15-19 (all use sFFLHD, b=3)

  - 15: SMED
  
  - 16: SMED with dimension distance adjustment
  
  - 17: Max weighted error (no integration)
  
  - 18: Max IntWerror reduction (1) using LOOEC for better error predictions
  
  - 19: Max Werror (17) using LOOEC for better error predictions
...
D <- 1
func <- logistic15
actual_des_func <- actual_des_func_grad_norm2_mean_logistic15 #get_num_actual_des_func_grad_norm2_mean(logistic15)
...
## Logistic, $E[g^2]$, $\alpha=1$, sFFLHD batch 3
...
## Logistic, $E[g^2]$, $\alpha=0$, sFFLHD batch 3
...
## Logistic, $E[g^2]$, $\alpha=1$, LHS batch 3
...
## Logistic, $E[g^2]$, $\alpha=0$, LHS batch 3
...
## Logistic, $E[g^2]$, sFFLHD non-adaptive batch 3
...
## Logistic, $E[g^2]$, sobel non-adaptive batch 3
...
## Logistic, $E[g^2]$, sFFLHD picking max SE batch 3
...
## Logistic, $E[g^2]$, sFFLHD picking max SE red batch 3
...
## Logistic, $E[g^2]$, $\alpha=1$, sFFLHD batch 1
...
## Logistic, $E[g^2]$, $\alpha=0$, sFFLHD batch 1
...
## Logistic, $E[g^2]$, $\alpha=1$, LHS batch 1
...
## Logistic, $E[g^2]$, $\alpha=0$, LHS batch 1
...
## Logistic, $E[g^2]$, SMED sFFLHD b=3
...
## Logistic, $E[g^2]$, SMED D-adjust sFFLHD b=3
...
## Logistic, $E[g^2]$, Max w*s sFFLHD b=3
...
## Logistic, $E[g^2]$, LOOEC IntWerror sFFLHD b=3
...
## Logistic, $E[g^2]$,  LOOEC max w*s, sFFLHD b=3
...
## Comparison of IntWerror means

```{r plotintwerror_iwe_means, cache=F}
mean_df <- plyr::ddply(.data = m_stats, .variables = c("n","name"), .fun = function(dd) {data.frame(n=dd$n[1],name=dd$name[1],actual_intwerror=mean(dd$actual_intwerror), mse=mean(dd$mse))})
ggplot(data=mean_df, aes(x=n, y=actual_intwerror, group=name, color=name)) + geom_line() + geom_point()  + scale_y_continuous(trans='log10')
```
