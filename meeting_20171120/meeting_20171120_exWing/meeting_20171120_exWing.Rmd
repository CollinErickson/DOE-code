...
n0 <- 3*D # 30 initial points
n_batches_to_run <- 24  # Gets up to 30+3*24=102
```
...
m3_runfunc <- function() {
  adapt.concept2.sFFLHD.R6$new(D=D,L=3,func=func, obj="desirability", des_func=des_func_grad_norm2_mean, 
                               ...
                               verbose=0, nconsider=c(Inf,10,10), nconsider_random=c(0),
                               # n0=n0
                               X0=MaxPro::MaxProLHD(n=D*3, p=D, itermax=1e4)$Design[sample(1:(D*3)),], #lhs::optimumLHS(n=6,k=2)
                               Xopts=MaxPro::MaxProLHD(n=D*30, p=D, itermax=1e4)$Design[sample(1:(D*30)),]
                               )
}
