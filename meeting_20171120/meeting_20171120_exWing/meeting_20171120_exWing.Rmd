---
title: "Meeting 11/20 Example Wing weight"
...
n_final <- 10*D # Final number of points, maybe 10D, more for lower D?
...
## Wing weight, $E[g^2]$, $\alpha=1$, sFFLHD batch 3
...
m1_runfunc <- function() {
  adapt.concept2.sFFLHD.R6$new(D=D,L=3,func=func, obj="desirability", des_func=des_func_grad_norm2_mean, 
                               ...
                               package=GP_package, design='sFFLHD_Lflex', 
...
## Wing weight, $E[g^2]$, sFFLHD non-adaptive batch 3
...
## Wing weight, $E[g^2]$, $\alpha=1$, sFFLHD batch 1
...
## Wing weight, $E[g^2]$, $\alpha=0$, sFFLHD batch 1
...
## Wing weight, $E[g^2]$, $\alpha=1$, LHS batch 1
...
## Wing weight, $E[g^2]$, $\alpha=0$, LHS batch 1
...
## Wing, $E[g^2]$, SMED D-adjust sFFLHD b=3
...
## Wing, $E[g^2]$, LOOEC IntWerror sFFLHD b=3

```{r m18_runfunc}
m18_runfunc <- function() {
  adapt.concept2.sFFLHD.R6$new(D=D,b=3,L=3,func=func, obj="desirability", des_func=des_func_grad_norm2_mean, 
                               alpha_des=1, weight_const=0, take_until_maxpvar_below=1, 
                               package="LOOEC-GauPro_kernel", design='sFFLHD_Lflex', 
                               selection_method="max_des_red_all", nugget=1e-6, 
                               actual_des_func=actual_des_func, 
                               verbose=0, nconsider=c(Inf,Inf,Inf), nconsider_random=c(0),
                               n0=n0
...
## Wing, $E[g^2]$,  LOOEC max w*s, sFFLHD b=3
...
for (i in 1:length(m3_runs)) {
  if (is.null(m3_runs[[i]])) { # Add this to add back ones that are completed but somehow got removed
    istring <- paste0('m',3,'e',i)
    if (exists(istring)) {
      m3_runs[[i]] <- eval(parse(text=paste0(istring,"$stats")))
      #stop("FINISH TRYING TO CHECK FOR EXISTING ONES")
    }
  }
...
for (i in 1:length(m4_runs)) {
  if (is.null(m4_runs[[i]])) { # Add this to add back ones that are completed but somehow got removed
    istring <- paste0('m',4,'e',i)
    if (exists(istring)) {
      m4_runs[[i]] <- eval(parse(text=paste0(istring,"$stats")))
      #stop("FINISH TRYING TO CHECK FOR EXISTING ONES")
    }
  }
...
for (i in 1:length(m9_runs)) {
  if (is.null(m9_runs[[i]])) { # Add this to add back ones that are completed but somehow got removed
    istring <- paste0('m',9,'e',i)
    if (exists(istring)) {
      m9_runs[[i]] <- eval(parse(text=paste0(istring,"$stats")))
    }
  }
...
for (i in 1:length(m10_runs)) {
  if (is.null(m10_runs[[i]])) { # Add this to add back ones that are completed but somehow got removed
    istring <- paste0('m',10,'e',i)
    if (exists(istring)) {
      m10_runs[[i]] <- eval(parse(text=paste0(istring,"$stats")))
    }
  }
...
for (i in 1:length(m11_runs)) {
  if (is.null(m11_runs[[i]])) { # Add this to add back ones that are completed but somehow got removed
    istring <- paste0('m',11,'e',i)
    if (exists(istring)) {
      m11_runs[[i]] <- eval(parse(text=paste0(istring,"$stats")))
    }
  }
...
for (i in 1:length(m12_runs)) {  
  if (is.null(m12_runs[[i]])) { # Add this to add back ones that are completed but somehow got removed
    istring <- paste0('m',12,'e',i)
    if (exists(istring)) {
      m12_runs[[i]] <- eval(parse(text=paste0(istring,"$stats")))
    }
  }
...
for (i in 1:length(m14_runs)) {  
  if (is.null(m14_runs[[i]])) { # Add this to add back ones that are completed but somehow got removed
    istring <- paste0('m',14,'e',i)
    if (exists(istring)) {
      m14_runs[[i]] <- eval(parse(text=paste0(istring,"$stats")))
    }
  }
...
```{r gatherresultsm15, cache=F}
m15_stats <- data.frame()
for (i in 1:length(m15_runs)) {
  if (!is.null(m15_runs[[i]])) {
    m15_stats <- rbind(m15_stats, 
                        data.frame(m=15, rp=i, name="SMED",
                                   iter=m15_runs[[i]]$stats$iter, 
                                   n=m15_runs[[i]]$stats$n,
                                   actual_intwerror=m15_runs[[i]]$stats$actual_intwerror, 
                                   mse=m15_runs[[i]]$stats$mse))
  }
}
m15_stats
```

```{r gatherresultsm16, cache=F}
m16_stats <- data.frame()
for (i in 1:length(m16_runs)) {
  if (!is.null(m16_runs[[i]])) {
    m16_stats <- rbind(m16_stats, 
                        data.frame(m=16, rp=i, name="SMED2",
                                   iter=m16_runs[[i]]$stats$iter, 
                                   n=m16_runs[[i]]$stats$n,
                                   actual_intwerror=m16_runs[[i]]$stats$actual_intwerror, 
                                   mse=m16_runs[[i]]$stats$mse))
  }
}
m16_stats
```
...
```{r gatherresultsm17, cache=F}
m17_stats <- data.frame()
for (i in 1:length(m17_runs)) {
  if (is.null(m17_runs[[i]])) { # Add this to add back ones that are completed but somehow got removed
    istring <- paste0('m',17,'e',i)
    if (exists(istring)) {
      # m17_runs[[i]] <- eval(parse(text=paste0(istring,"$stats")))
      m17_runs[[i]] <- eval(parse(text=paste0(istring)))
    }
  }
  if (!is.null(m17_runs[[i]])) {
    m17_stats <- rbind(m17_stats, 
                        data.frame(m=17, rp=i, name="swb3a1",
                                   iter=m17_runs[[i]]$stats$iter, 
                                   n=m17_runs[[i]]$stats$n,
                                   actual_intwerror=m17_runs[[i]]$stats$actual_intwerror, 
                                   mse=m17_runs[[i]]$stats$mse))
  }
}
m17_stats
```

```{r gatherresultsm18, cache=F}
m18_stats <- data.frame()
for (i in 1:length(m18_runs)) {
  if (is.null(m18_runs[[i]])) { # Add this to add back ones that are completed but somehow got removed
    istring <- paste0('m',18,'e',i)
    if (exists(istring)) {
      # m18_runs[[i]] <- eval(parse(text=paste0(istring,"$stats")))
      m18_runs[[i]] <- eval(parse(text=paste0(istring)))
    }
  }
  if (!is.null(m18_runs[[i]])) {
    m18_stats <- rbind(m18_stats, 
                        data.frame(m=18, rp=i, name="sWb3a1L",
                                   iter=m18_runs[[i]]$stats$iter, 
                                   n=m18_runs[[i]]$stats$n,
                                   actual_intwerror=m18_runs[[i]]$stats$actual_intwerror, 
                                   mse=m18_runs[[i]]$stats$mse))
  }
}
m18_stats
```

```{r gatherresultsm19, cache=F}
m19_stats <- data.frame()
for (i in 1:length(m19_runs)) {
  if (is.null(m19_runs[[i]])) { # Add this to add back ones that are completed but somehow got removed
    istring <- paste0('m',19,'e',i)
    if (exists(istring)) {
      # m19_runs[[i]] <- eval(parse(text=paste0(istring,"$stats")))
      m19_runs[[i]] <- eval(parse(text=paste0(istring)))
    }
  }
  if (!is.null(m19_runs[[i]])) {
    m19_stats <- rbind(m19_stats, 
                        data.frame(m=19, rp=i, name="swb3a1L",
                                   iter=m19_runs[[i]]$stats$iter, 
                                   n=m19_runs[[i]]$stats$n,
                                   actual_intwerror=m19_runs[[i]]$stats$actual_intwerror, 
                                   mse=m19_runs[[i]]$stats$mse))
  }
}
m19_stats
```
...
m_stats <- rbind(m1_stats, m2_stats, m3_stats, m4_stats, m5_stats, m6_stats, m7_stats, m8_stats,
                 ...
                 m15_stats, m16_stats, m17_stats, m18_stats, m19_stats)
```
...
p1 <- p1 + theme(axis.text.x=element_text(angle=90,hjust=1, vjust=.5))  # Rotate x-axis labels
p1
```
