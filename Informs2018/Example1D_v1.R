...
make1Dplots2 <- function(f, xlist, ylim,#x=c(0,.5,1), x2=c(.25,.75),
                         ...
                         Proposed="IMVSE",
                         runadapt=FALSE,batchsize, nadapt,
                         savename) {
  if (sameplot) {
    par(mfrow=c(2,2))
  }
  # curve(f)
  x <- matrix(xlist[[1]], ncol=1)
  # set.seed(0)
  if (missing(theta)) {
  ...
  # }
  # 
  if (runadapt) {nloops <- nadapt} else {nloops <- length(xlist)}
  for (i in 1:nloops) {
    if (i >= 2) {
      if (runadapt) {
        xopts <- seq(0,1,l=1e3)
        crit <- crit_imse
        xnew <- xopts[order(crit(xopts))[1:batchsize]]
      } else {
        xnew <- xlist[[i]]
      }
      if (no_update)
        gp$update(Xnew = as.matrix(xnew, ncol=1), Znew=c(f(xnew)), no_update=T)
      else
        gp$update(Xnew = as.matrix(xnew, ncol=1), Znew=c(f(xnew)))
      x <- c(x, xnew) # for plotting points
    }
    ...
    if (colorplot) {
      ...
      if (plotknown) points(a, a_ytrue, col=1, type='l', lwd=3)
      points(x, f(x), pch=19, cex=2)
      # if (plotknown) legend(x = 'topleft', legend=c("Actual", "Predicted", "95% interval"), fill=c(1,2,3))
      # else legend(x = 'topleft', legend=c("Predicted", "95% interval"), fill=c(2,3))
    } else {
      # Black/white/gray, use line types and shading to distinguish
      plot(a, a_ypred, col=1, lty=2, type='l', xlab='Input (x)', ylab='Output (y)', ylim=c(min1, max1), lwd=lwd1,
           panel.first = {rect(a,a_ylower,a,a_yupper, col = "gray", density = 2)})
      # rect(a,a_ylower,a,a_yupper, col = "gray", density = 2)
      if (plotknown) points(a, a_ytrue, col=1, type='l', lty=1, lwd=3)
      points(x, f(x), pch=19, cex=2)
      # browser()
      # if (plotknown) legend(x = 'topleft', legend=c("Actual", "Predicted"), lty=c(1, 2), lwd=2)
      # else legend(x = 'topleft', legend=c("Predicted"), lty=c(2), lwd=2)
    }
...
# make1Dplots2(matt,  sameplot = F,
             # xlist=list(c(0,.7,1), x2=c(.2,.4,.83,.6)))
make1Dplots2(matt,  sameplot = F, ylim=c(-4,4),colorplot = F,
             xlist=list(c(0,1), c(.4,.7), c(.2, .6), c(.83,.3))
             )#savename="GradAdapt1DEx1_")
make1Dplots2(matt,  sameplot = F, ylim=c(-4,4),colorplot = F,
             xlist=list(c(0,1), c(.4,.7), c(.2, .6), c(.83,.3)),
             runadapt = T, nadapt=5, batchsize=2
)
