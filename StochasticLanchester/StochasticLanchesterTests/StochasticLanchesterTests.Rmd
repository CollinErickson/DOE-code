...
avg_grad_norm2_mean <- function(x, ds, reps=1e3) {
  ...
  mean(
    replicate(reps, {
      x4[notds] <- runif(2)
      a$mod$mod.extra$GauPro$mod$grad_norm2_mean(matrix(x4, ncol=4))
    })
  )
}
avg_grad_norm2_mean_lhd <- function(x, ds, reps=1e3) {
  ...
  X4[, ds[2]] <- x[2]
  X4[, notds] <- XX
  mean(a$mod$mod.extra$GauPro$mod$grad_norm2_mean(X4))
}
...
cf_highdim(function(x)a$mod$mod.extra$GauPro$mod$grad_norm2_mean(matrix(x, ncol=4)), 4, average=T, average_reps = 1e2, average_matrix = T, n=8)
```
