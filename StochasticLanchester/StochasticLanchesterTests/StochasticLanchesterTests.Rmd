...
cf_highdim(rler5$mod$predict, D=4, pts=rler5$X)
...
avg_grad_norm2_mean <- function(x, ds, reps=1e3) {
  x4 <- numeric(4)
  x4[ds] <- x
  ...
  mean(
    replicate(reps, {
      x4[notds] <- runif(2)
      rler5$mod$mod.extra$GauPro$mod$grad_norm2_mean(matrix(x4, ncol=4))
    })
  )
}
avg_grad_norm2_mean_lhd <- function(x, ds, reps=1e3) {
  x4 <- numeric(4)
  x4[ds] <- x
  notds <- setdiff(1:4, ds)
  XX <- simple.LHS(reps, 2)
  X4 <- matrix(nrow=reps, ncol=4)
  X4[, ds[1]] <- x[1]
  X4[, ds[2]] <- x[2]
  X4[, notds] <- XX
  mean(rler5$mod$mod.extra$GauPro$mod$grad_norm2_mean(X4))
}
summary(replicate(10, avg_grad_norm2_mean_lhd(c(.4,.4), 1:2, reps=1e4)))
```

```{r, eval=FALSE, include=F}
cf(function(x) avg_grad_norm2_mean_lhd(x, 1:2, reps=1e4))
cf(function(x) avg_grad_norm2_mean_lhd(x, 3:4, reps=1e2), n=8)
cf_highdim(function(x)rler5$mod$mod.extra$GauPro$mod$grad_norm2_mean(matrix(x, ncol=4)), 4, average=T, average_reps = 1e2, n=8)
```

#### Mean, averaged over other two dimensions

```{r}
cf_highdim(rler5$mod$predict, D=4, pts=rler5$X, average=TRUE)
```
